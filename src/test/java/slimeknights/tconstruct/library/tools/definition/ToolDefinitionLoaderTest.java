...
class ToolDefinitionLoaderTest extends BaseMcTest {
  ...
  private static void checkFullNonParts(ToolDefinitionData data) {
    ...
    // actions
    ItemAbilityToolHook actions = data.getHook(ToolHooks.TOOL_ACTION);
    assertThat(actions).isInstanceOf(ItemAbilitiesModule.class);
    assertThat(((ItemAbilitiesModule) actions).actions()).hasSize(2);
