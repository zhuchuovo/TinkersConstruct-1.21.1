...
public class MixerAlloyTank implements IMutableAlloyTank {
      ...
      if (currentTanks > 0) {
        ...
        for (Direction direction : Direction.values()) {
          if (direction != Direction.DOWN) {
            IFluidHandler handler = inputs.getOrDefault(direction, EmptyFluidHandler.INSTANCE);
        ...
        if (direction != Direction.DOWN && !inputs.containsKey(direction)) {
          ...
          if (world.getBlockState(target).is(TinkerTags.Blocks.ALLOYER_TANKS)) {
            ...
            if (capability != null) {
              ...
              inputs.put(direction, EmptyFluidHandler.INSTANCE);
            }
          }
        }
      }
  ...
  public void refresh(Direction direction, boolean checkInput) {
    ...
    if (!checkInput || (inputs.containsKey(direction) && inputs.get(direction) != EmptyFluidHandler.INSTANCE)) {
      currentTanks--;
    }
