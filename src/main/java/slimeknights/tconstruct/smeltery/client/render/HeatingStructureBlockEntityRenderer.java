    ...
    if (max != 0) {
      ...
      for (int i = 0; i < inventory.getSlots(); i++) {
        ...
        if (!stack.isEmpty()) {
          ...
          itemRenderer.render(stack, TinkerItemDisplays.MELTER.getValue(), false, matrices, buffer, LevelRenderer.getLightColor(world, itemPos), OverlayTexture.NO_OVERLAY, model);
          matrices.popPose();
