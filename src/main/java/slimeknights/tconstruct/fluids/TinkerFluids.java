...
public final class TinkerFluids extends TinkerModule {
  ...
  public static final FlowingFluidObject<SlimeFluid> earthSlime = FLUIDS.registerSlime("earth_slime").type(slime("earth_slime").temperature(350)).bucket().block(createEffect(MapColor.GRASS, 0, () -> new MobEffectInstance(TinkerEffects.bouncy.<MobEffect>getHolderAs(), 5*20))).commonTag("slime").flowing(SlimeFluid.Source::new, SlimeFluid.Flowing::new);
  public static final FlowingFluidObject<SlimeFluid> skySlime   = FLUIDS.registerSlime("sky_slime"  ).type(slime("sky_slime"  ).temperature(310)).bucket().block(createEffect(MapColor.DIAMOND, 0, () -> new MobEffectInstance(TinkerEffects.ricochet.<MobEffect>getHolderAs(), 5*20))).flowing(SlimeFluid.Source::new, SlimeFluid.Flowing::new);
  public static final FlowingFluidObject<SlimeFluid> enderSlime = FLUIDS.registerSlime("ender_slime").type(slime("ender_slime").temperature(370)).bucket().block(createEffect(MapColor.COLOR_PURPLE, 0, () -> new MobEffectInstance(TinkerEffects.enderference.<MobEffect>getHolderAs(), 5 * 20))).flowing(SlimeFluid.Source::new, SlimeFluid.Flowing::new);
  ...
  public static final FlowingFluidObject<BaseFlowingFluid> moltenEnder    = FLUIDS.registerStone("molten_ender").type(hot("molten_ender").temperature(777).lightLevel(5)).block(createEffect(MapColor.PLANT, 5, () -> new MobEffectInstance(TinkerEffects.enderference.<MobEffect>getHolderAs(), 5 * 20))).bucket().commonTag("ender").flowing();
  ...
  void gatherData(final GatherDataEvent event) {
    // Runtime-first 1.21 port: providers are migrated separately.
  }

  @SubscribeEvent
  void registerCapabilities(final RegisterCapabilitiesEvent event) {
    event.registerItem(Capabilities.FluidHandler.ITEM,
      (stack, ignored) -> new ConstantFluidContainerWrapper(
        new FluidStack(powderedSnow.get(), FluidType.BUCKET_VOLUME), stack, Items.BUCKET.getDefaultInstance()),
      Items.POWDER_SNOW_BUCKET);
    event.registerItem(Capabilities.FluidHandler.ITEM,
      (stack, ignored) -> ((FluidContainerFoodItem)stack.getItem()).createFluidHandler(stack), venomBottle);
    slimeBottle.forEach(item -> event.registerItem(Capabilities.FluidHandler.ITEM,
      (stack, ignored) -> ((FluidContainerFoodItem)stack.getItem()).createFluidHandler(stack), item));
    event.registerItem(Capabilities.FluidHandler.ITEM,
      (stack, ignored) -> ((MagmaBottleItem)stack.getItem()).createFluidHandler(stack), magmaBottle);
    event.registerItem(Capabilities.FluidHandler.ITEM,
      (stack, ignored) -> ((PotionBucketItem)stack.getItem()).createFluidHandler(stack), potion.asItem());
  }

  @SubscribeEvent
  void commonSetup(final FMLCommonSetupEvent event) {
    event.enqueueWork(() -> {
      var waterInteractions = CauldronInteraction.WATER.map();
      var emptyInteractions = CauldronInteraction.EMPTY.map();
      waterInteractions.put(splashBottle.get(), new FillBottle(Items.SPLASH_POTION));
      waterInteractions.put(lingeringBottle.get(), new FillBottle(Items.LINGERING_POTION));
      waterInteractions.put(Items.SPLASH_POTION,    new EmptyBottleIntoWater(splashBottle,    waterInteractions.get(Items.SPLASH_POTION)));
      waterInteractions.put(Items.LINGERING_POTION, new EmptyBottleIntoWater(lingeringBottle, waterInteractions.get(Items.LINGERING_POTION)));
      emptyInteractions.put(Items.SPLASH_POTION,    new EmptyBottleIntoEmpty(splashBottle,    emptyInteractions.get(Items.SPLASH_POTION)));
      emptyInteractions.put(Items.LINGERING_POTION, new EmptyBottleIntoEmpty(lingeringBottle, emptyInteractions.get(Items.LINGERING_POTION)));
    });
  ...
  /** Registers custom bottle transformations on the 1.21 reload-aware brewing builder. */
  private void registerBrewingRecipes(RegisterBrewingRecipesEvent event) {
    var builder = event.getBuilder();
    builder.addRecipe(new BottleBrewingRecipe(Ingredient.of(Items.GLASS_BOTTLE), Ingredient.of(Items.GUNPOWDER), new ItemStack(splashBottle)));
    builder.addRecipe(new BottleBrewingRecipe(Ingredient.of(MantleTags.Items.SPLASH_BOTTLE), Ingredient.of(Items.DRAGON_BREATH), new ItemStack(lingeringBottle)));
    for (SlimeType slime : SlimeType.values()) {
      builder.addRecipe(Ingredient.of(Items.GLASS_BOTTLE), Ingredient.of(TinkerWorld.congealedSlime.get(slime)), new ItemStack(slimeBottle.get(slime)));
    }
    builder.addRecipe(Ingredient.of(Items.GLASS_BOTTLE), Ingredient.of(Blocks.MAGMA_BLOCK), new ItemStack(magmaBottle));
  }
