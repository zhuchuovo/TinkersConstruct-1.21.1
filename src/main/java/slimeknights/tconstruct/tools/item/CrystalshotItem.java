...
public class CrystalshotItem extends ArrowItem {
  ...
  public AbstractArrow createArrow(Level pLevel, ItemStack pStack, LivingEntity pShooter, ItemStack weapon) {
    ...
    CompoundTag tag = ItemStackUtil.getTag(pStack);
  ...
  @Override
  public boolean isInfinite(ItemStack stack, ItemStack bow, LivingEntity living) {
    var infinity = living.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.INFINITY);
  ...
  public static ItemStack withVariant(String variant, int size) {
    ...
    ItemStackUtil.getOrCreateTag(stack).putString(TAG_VARIANT, variant);
    return stack;
  }
  ...
  public static class CrystalshotEntity extends AbstractArrow {
    ...
    public CrystalshotEntity(Level level, LivingEntity shooter, ItemStack pickup, ItemStack weapon) {
      super(TinkerTools.crystalshotEntity.get(), shooter, level, pickup, weapon);
      setSoundEvent(Sounds.CRYSTALSHOT.getSound());
    }
