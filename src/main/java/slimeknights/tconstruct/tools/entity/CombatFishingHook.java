...
public class CombatFishingHook extends FishingHook implements ProjectileWithKnockback, ProjectileWithPower {
      ...
      if (power > 0 && !collectable) {
        ...
        if (target.hurt(source, damage)) {
          if (!this.level().isClientSide && owner instanceof LivingEntity ownerLiving) {
            if (targetLiving != null && this.level() instanceof ServerLevel server) {
              EnchantmentHelper.doPostAttackEffects(server, targetLiving, source);
            }
  ...
  private void pullGrapple(Entity owner) {
    ...
    if (isDrill() && owner instanceof Player player) {
      ItemStack spinItem = player.getMainHandItem();
      if (!spinItem.canPerformAction(ItemAbilities.FISHING_ROD_CAST)) {
        spinItem = player.getOffhandItem();
      }
      player.startAutoSpinAttack(20, 8.0F, spinItem);
    }
