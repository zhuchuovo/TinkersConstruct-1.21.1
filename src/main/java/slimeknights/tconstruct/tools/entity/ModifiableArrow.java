...
public class ModifiableArrow extends AbstractArrow implements ToolProjectile, ReusableProjectile {
  ...
  /** Additional knockback supplied by modifiers and vanilla bow hooks. */
  ...
  public ModifiableArrow(Level level, LivingEntity shooter, @Nullable ItemStack weapon) {
    super(TinkerTools.materialArrow.get(), shooter, level, new ItemStack(TinkerTools.arrow), weapon);
  }
  ...
  public void doKnockback(LivingEntity target, DamageSource source) {
    ...
    if (knockback > 0) {
      ...
      Vec3 movement = getDeltaMovement().multiply(1, 0, 1).normalize().scale(knockback * 0.6 * resistance);
      if (movement.lengthSqr() > 0) {
        target.push(movement.x, 0.1, movement.z);
      }
    }
  }
