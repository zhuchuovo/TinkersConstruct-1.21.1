...
public class ThrownShuriken extends Projectile implements ToolProjectile, ProjectileWithPower, ProjectileWithKnockback, ReusableProjectile {
  ...
  public void tick() {
    ...
    if (hit.getType() == HitResult.Type.BLOCK) {
      ...
      if (state.getBlock() instanceof Portal portal && this.canUsePortal(false)) {
        if (!(state.is(Blocks.END_GATEWAY)) || (this.level().getBlockEntity(pos) instanceof TheEndGatewayBlockEntity gateway && !gateway.isCoolingDown())) {
          this.setAsInsidePortal(portal, pos);
          if (this.level().getBlockEntity(pos) instanceof TheEndGatewayBlockEntity gateway) {
            TheEndGatewayBlockEntity.triggerCooldown(this.level(), pos, state, gateway);
          }
        }
        teleported = true;
      }
    ...
    if (type != HitResult.Type.MISS && !teleported) {
      if (!stack.isEmpty() && type == HitResult.Type.ENTITY && ModifierUtil.canPerformAction(getTool(), TinkerItemAbilities.SHIELD_DISABLE)) {
  ...
  public Component getDisplayName() {
    return getDisplayTool().getHoverName();
  }
