...
public class ToolEvents {
  ...
  static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
    ...
    double armorMultiplier = player.getAttributeValue(TinkerAttributes.MINING_SPEED_MULTIPLIER.getHolder()) + ArmorStatModule.getStat(player, TinkerDataKeys.MINING_SPEED);
  ...
  static void livingHurt(LivingIncomingDamageEvent event) {
    ...
    if (attacker instanceof LivingEntity living) {
      ...
      if (attacker.isCrouching()) {
        double crouchMultiplier = living.getAttributeValue(TinkerAttributes.CROUCH_DAMAGE_MULTIPLIER.getHolder());
        crouchMultiplier += ArmorStatModule.getStat(living, TinkerDataKeys.CROUCH_DAMAGE);
      ...
      if (!source.is(DamageTypeTags.BYPASSES_ARMOR)) {
        ...
        if (damageMissed > 0 && entity instanceof Player) {
          for (EquipmentSlot slotType : ModifiableArmorMaterial.ARMOR_SLOTS) {
            ...
            if (tool != null) {
              if (armorStack.canBeHurtBy(source)) {
