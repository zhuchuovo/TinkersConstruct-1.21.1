...
public class ModifierEvents {
  ...
  static void onKnockback(LivingKnockBackEvent event) {
    ...
    Optional<TinkerDataCapability.Holder> dataCap = TinkerDataCapability.getOptional(entity);
    double knockback = entity.getAttributeValue(TinkerAttributes.KNOCKBACK_MULTIPLIER.getHolder())
  ...
  static void onLivingFall(LivingFallEvent event) {
    LivingEntity entity = event.getEntity();
    double boost = entity.getAttributeValue(TinkerAttributes.SAFE_FALL_DISTANCE.getHolder()) + ArmorStatModule.getStat(entity, TinkerDataKeys.JUMP_BOOST);
    if (boost != 0) {
  ...
  public static void onLivingJump(LivingJumpEvent event) {
    LivingEntity entity = event.getEntity();
    double boost = entity.getAttributeValue(TinkerAttributes.JUMP_BOOST.getHolder()) + ArmorStatModule.getStat(entity, TinkerDataKeys.JUMP_BOOST);
    if (boost > 0) {
    ...
    if (!entity.level().getGameRules().getBoolean(GameRules.RULE_KEEPINVENTORY) && entity instanceof Player player && !(player instanceof FakePlayer)) {
      ...
      for (int i = 0; i < hotbarSize; i++) {
        ...
        if (!stack.isEmpty() && (soulBelt || ModifierUtil.checkVolatileFlag(stack, SOULBOUND))) {
          ItemStackUtil.getOrCreateTag(stack).putInt(MantleEvents.SOULBOUND_SLOT, i);
        }
      }
  ...
  static void beforeBlockBreak(BlockDropsEvent event) {
    ...
    double bonus = player.getAttributeValue(TinkerAttributes.EXPERIENCE_MULTIPLIER.getHolder())
  ...
  static void onExperienceDrop(LivingExperienceDropEvent event) {
    ...
    LivingEntity entity = event.getEntity();
    MobEffectInstance instance = entity.getEffect(TinkerEffects.experienced.getHolderAs());
    ...
    if (player != null) {
      multiplier += player.getAttributeValue(TinkerAttributes.EXPERIENCE_MULTIPLIER.getHolder()) + ArmorStatModule.getStat(player, TinkerDataKeys.EXPERIENCE);
    }
  ...
  static void onCritical(CriticalHitEvent event) {
    ...
    LivingEntity living = event.getEntity();
      ...
      Holder<Attribute> attribute = TinkerAttributes.CRITICAL_DAMAGE.getHolder();
  ...
  static void onPotionStart(MobEffectEvent.Added event) {
    ...
    if (!newEffect.isInfiniteDuration() && !newEffect.getCures().isEmpty()) {
      ...
      LivingEntity entity = event.getEntity();
      double multiplier = entity.getAttributeValue(beneficial ? TinkerAttributes.GOOD_EFFECT_DURATION.getHolder() : TinkerAttributes.BAD_EFFECT_DURATION.getHolder())
  ...
  static void bounceOnFall(LivingFallEvent event) {
    ...
    if (living.getAttributeValue(TinkerAttributes.BOUNCY.getHolder()) < 1) {
      return;
    }
        ...
        if (target.hurt(damageSource, (float) damage)) {
          ...
          if (!level.isClientSide && arrow.getDeltaMovement().lengthSqr() < 1.0E-7D) {
            if (arrow.pickup == AbstractArrow.Pickup.ALLOWED) {
              arrow.spawnAtLocation(arrow.getPickupItemStackOrigin().copy(), 0.1F);
            }
  ...
  static void onTeleport(EntityTeleportEvent event) {
    if (event.getEntity() instanceof LivingEntity living && living.hasEffect(TinkerEffects.enderference.getHolderAs())) {
      event.setCanceled(true);
    }
  }
