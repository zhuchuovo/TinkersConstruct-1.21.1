...
public class CombatFishingHookRenderer extends EntityRenderer<CombatFishingHook> {
  ...
  /** Vanilla fishing-line vertex helper, copied as it is private in 1.21. */
  private static void stringVertex(float x, float y, float z, VertexConsumer consumer, PoseStack.Pose pose, float fraction, float nextFraction) {
    float px = x * fraction;
    ...
    consumer.addVertex(pose, px, py, pz).setColor(0xFF000000).setNormal(pose, nx / length, ny / length, nz / length);
  }
}
