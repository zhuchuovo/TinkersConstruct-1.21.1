...
public class EnchantmentConvertingRecipe extends AbstractWorktableRecipe {
  ...
  private Map<Holder<Enchantment>,Integer> getEnchantments(ItemStack stack) {
    ...
    EnchantmentHelper.getEnchantmentsForCrafting(stack).entrySet().forEach(entry -> enchantments.put(entry.getKey(), entry.getIntValue()));
    return enchantments;
  }
      ...
      if (matchBook) {
        ...
        displayModifiers = ModifierManager.INSTANCE.getEquivalentEnchantments(modifiers::contains)
          .flatMap(enchantment -> IntStream.rangeClosed(1, enchantment.value().getMaxLevel())
            .mapToObj(level -> new ModifierEntry(Objects.requireNonNullElse(ModifierManager.INSTANCE.get(enchantment.value()), defaultModifier), level)))
          .toList();
      } else {
  ...
  public void updateInputs(LazyToolStack result, ITinkerableContainer.Mutable inv, ModifierEntry selected, boolean isServer) {
    ...
    if (returnInput && isServer) {
      ...
      for (Iterator<Entry<Holder<Enchantment>,Integer>> iterator = enchantments.entrySet().iterator(); iterator.hasNext();) {
        Entry<Holder<Enchantment>,Integer> entry = iterator.next();
      ...
      if (matchBook && enchantments.isEmpty()) {
        ...
        enchantments.forEach(updated::set);
        EnchantmentHelper.setEnchantments(unenchanted, updated.toImmutable());
      }
