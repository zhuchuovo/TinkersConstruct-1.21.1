...
public class AntigravityEffect extends TinkerEffect {
  ...
  public AntigravityEffect() {
    ...
    NeoForge.EVENT_BUS.addListener((MobEffectEvent.Remove event) -> onEffectRemoved(event.getEntity(), event.getEffect()));
    NeoForge.EVENT_BUS.addListener((MobEffectEvent.Expired event) -> onEffectRemoved(event.getEntity(), event.getEffectInstance().getEffect()));
  }
    ...
    if (living.getAttributeValue(Attributes.GRAVITY) < 0) {
      ...
      if (living.verticalCollision && !living.verticalCollisionBelow) {
        if (lastVelocity.twoTicks > 0.1f) {
          ...
          float height = (float)(lastVelocity.twoTicks * 10 - 3 - living.getAttributeValue(TinkerAttributes.SAFE_FALL_DISTANCE.getHolder()) - TinkerEffect.getLevel(living, MobEffects.JUMP));
          if (height > 0.0F) {
      ...
      if (living.onClimbable()) {
        ...
        if (living.horizontalCollision || living.getDeltaMovement().y < 0) {
          y = -0.2;
        }
  ...
  private void onEffectRemoved(LivingEntity living, Holder<MobEffect> effect) {
