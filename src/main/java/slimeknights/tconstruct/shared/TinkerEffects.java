...
public class TinkerEffects extends TinkerModule {
  ...
  // slimy potions
  public static final RegistryObject<TinkerEffect> experienced = MOB_EFFECTS.register("experienced", () -> new TinkerEffect(MobEffectCategory.BENEFICIAL, 0x82c873, true).addAttributeModifier(TinkerAttributes.EXPERIENCE_MULTIPLIER.get(), "ccffb654-9988-451e-9539-f74934274df1", 0.25f, Operation.ADD_MULTIPLIED_BASE));
  public static final RegistryObject<TinkerEffect> ricochet = MOB_EFFECTS.register("ricochet", () -> new TinkerEffect(MobEffectCategory.NEUTRAL, 0x01cbcd, true).addAttributeModifier(TinkerAttributes.KNOCKBACK_MULTIPLIER.get(), "58a4bc13-366f-4f76-82f5-705451498c24", 0.5f, Operation.ADD_MULTIPLIED_BASE));
  ...
  // slimy cakes
  public static final RegistryObject<TinkerEffect> bouncy = MOB_EFFECTS.register("bouncy", () -> new TinkerEffect(MobEffectCategory.BENEFICIAL, 0x71AC63, true).addAttributeModifier(TinkerAttributes.BOUNCY.get(), "5de036ed-bc47-4965-9348-64c3ab5c8ae8", 1, Operation.ADD_VALUE));
  public static final RegistryObject<TinkerEffect> doubleJump = MOB_EFFECTS.register("double_jump", () -> new TinkerEffect(MobEffectCategory.BENEFICIAL, 0xA99B87, true).addAttributeModifier(TinkerAttributes.JUMP_COUNT.get(), "9863601a-9d4a-4708-b348-4bf9fe6c0bbd", 1, Operation.ADD_VALUE));
  ...
  public TinkerEffects() {
    POTIONS.register(TConstruct.getModBus());
    NeoForge.EVENT_BUS.addListener((RegisterBrewingRecipesEvent event) -> registerBrewingRecipes(event));
  }
  ...
  private static void brewing(PotionBrewing.Builder builder, EnumObject<PotionType,Potion> potion, Holder<Potion> base, net.minecraft.world.item.Item ingredient) {
    Potion normal = potion.get(PotionType.NORMAL);
    Holder<Potion> normalHolder = BuiltInRegistries.POTION.wrapAsHolder(normal);
    builder.addMix(base, ingredient, normalHolder);
    Potion longer = potion.getOrNull(PotionType.LONG);
    if (longer != null) {
      builder.addMix(normalHolder, Items.REDSTONE, BuiltInRegistries.POTION.wrapAsHolder(longer));
    }
    Potion strong = potion.getOrNull(PotionType.STRONG);
    if (strong != null) {
      builder.addMix(normalHolder, Items.GLOWSTONE_DUST, BuiltInRegistries.POTION.wrapAsHolder(strong));
    }
  }
  ...
  public static boolean canHitWithProjectile(@Nullable LivingEntity living) {
    return living == null || living.getType() != EntityType.ENDERMAN || living.hasEffect(enderference.getHolderAs());
  }
  ...
  public static boolean needsEnderferenceOverride(@Nullable Entity entity) {
    return entity != null && entity.getType() == EntityType.ENDERMAN && entity instanceof LivingEntity living && living.hasEffect(enderference.getHolderAs());
  }
