...
public class TinkerAttributes {
  ...
  public TinkerAttributes() {
    ATTRIBUTES.register(TConstruct.getModBus());
  }
  ...
  void addAttributes(EntityAttributeModificationEvent event) {
    // player attributes
    event.add(EntityType.PLAYER, USE_ITEM_SPEED.getHolder());
    event.add(EntityType.PLAYER, CRITICAL_DAMAGE.getHolder());
    event.add(EntityType.PLAYER, MINING_SPEED_MULTIPLIER.getHolder());
    event.add(EntityType.PLAYER, EXPERIENCE_MULTIPLIER.getHolder());
    event.add(EntityType.PLAYER, JUMP_COUNT.getHolder());
    // general attributes
  ...
  private static void addToAll(EntityAttributeModificationEvent event, RegistryObject<Attribute> attribute, double defaultValue) {
    Holder<Attribute> attr = attribute.getHolder();
