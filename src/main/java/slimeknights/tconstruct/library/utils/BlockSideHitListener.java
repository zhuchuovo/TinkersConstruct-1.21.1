...
public class BlockSideHitListener {
  ...
  private static void breakBlock(BlockEvent.BreakEvent event) {
    ...
    int experience = event.getState().getExpDrop(level, pos, level.getBlockEntity(pos), event.getPlayer(), event.getPlayer().getMainHandItem());
    LAST_XP.put(event.getPlayer().getUUID(), experience);
  }
