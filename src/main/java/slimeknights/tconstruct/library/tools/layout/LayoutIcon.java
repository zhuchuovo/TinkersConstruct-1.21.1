...
public abstract class LayoutIcon {
  ...
  public static LayoutIcon read(FriendlyByteBuf buffer) {
    ...
    switch (type) {
      ...
      case ITEM: {
        ItemStack stack = ItemStack.STREAM_CODEC.decode((RegistryFriendlyByteBuf)buffer);
        return new ItemStackIcon(stack);
      }
      case PATTERN: {
  ...
  protected static class Serializer implements JsonSerializer<LayoutIcon>, JsonDeserializer<LayoutIcon> {
    ...
    public LayoutIcon deserialize(JsonElement json, java.lang.reflect.Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
      ...
      if (object.has("item")) {
        var item = BuiltInRegistries.ITEM.get(ResourceLocation.parse(GsonHelper.getAsString(object, "item")));
        if (item == null) throw new JsonSyntaxException("Unknown layout icon item " + object.get("item"));
        ItemStack stack = new ItemStack(item, GsonHelper.getAsInt(object, "count", 1));
        if (object.has("nbt")) {
          try {
            ItemStackUtil.setTag(stack, TagParser.parseTag(GsonHelper.getAsString(object, "nbt")));
          } catch (com.mojang.brigadier.exceptions.CommandSyntaxException e) {
            throw new JsonSyntaxException("Invalid layout icon NBT", e);
          }
        }
        return new ItemStackIcon(stack);
      }
