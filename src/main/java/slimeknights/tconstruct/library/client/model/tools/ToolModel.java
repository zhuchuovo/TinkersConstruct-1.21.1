  ...
  public static final class MaterialOverrideHandler extends NestedOverrides {
    ...
    public BakedModel resolve(BakedModel originalModel, ItemStack stack, @Nullable ClientLevel world, @Nullable LivingEntity entity, int seed) {
      ...
      if (ammoKey != null && persistentData.contains(ammoKey, Tag.TAG_COMPOUND)) {
        net.minecraft.world.level.Level level = world != null ? world : net.minecraft.client.Minecraft.getInstance().level;
        ammo = level == null ? ItemStack.EMPTY : ItemStack.parse(level.registryAccess(), persistentData.getCompound(ammoKey)).orElse(ItemStack.EMPTY);
        builder.add(ammo.getItem());
        CompoundTag tag = slimeknights.tconstruct.library.utils.ItemStackUtil.getTag(ammo);
        if (tag != null) {
          builder.add(tag);
        }
      } else {
