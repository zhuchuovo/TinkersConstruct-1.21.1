    ...
    if (json.has("fluid")) {
      ...
      if (fluidElement.isJsonObject()) {
        ...
        if (fluidObject.has("nbt")) {
          try {
            tag = TagParser.parseTag(fluidObject.get("nbt").isJsonPrimitive()
              ? fluidObject.get("nbt").getAsString() : fluidObject.get("nbt").toString());
          } catch (com.mojang.brigadier.exceptions.CommandSyntaxException e) {
            throw new com.google.gson.JsonParseException("Invalid fluid NBT", e);
          }
        }
      } else {
      ...
      FluidStackUtil.setTag(fluidStack, tag);
    }
