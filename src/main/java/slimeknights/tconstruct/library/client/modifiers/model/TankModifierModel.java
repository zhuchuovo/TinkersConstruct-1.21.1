...
public class TankModifierModel implements ModifierModel {
  ...
  public Object getCacheKey(IToolStackView tool, ModifierEntry modifier) {
    ...
    if (!fluid.isEmpty()) {
      return new CacheKey(fluid.getFluid(), slimeknights.tconstruct.library.utils.FluidStackUtil.getTag(fluid), fluid.getAmount() + tolerance < helper.getCapacity(tool));
    }
    return null;
  }
