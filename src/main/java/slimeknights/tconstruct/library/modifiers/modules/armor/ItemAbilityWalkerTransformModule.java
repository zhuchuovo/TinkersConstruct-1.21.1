 ...
 */
public record ItemAbilityWalkerTransformModule(ItemAbility action, SoundEvent sound, LevelingValue radius, ModifierCondition<IToolStackView> condition) implements ModifierModule, ArmorWalkRadiusModule<MutableUseOnContext>, ItemAbilityModifierHook, ConditionalModule<IToolStackView> {
  private static final List<ModuleHook<?>> DEFAULT_HOOKS = HookProvider.<ItemAbilityWalkerTransformModule>defaultHooks(ModifierHooks.BOOT_WALK, ModifierHooks.TOOL_ACTION);
  public static final RecordLoadable<ItemAbilityWalkerTransformModule> LOADER = RecordLoadable.create(
    Loadables.TOOL_ACTION.requiredField("tool_action", ItemAbilityWalkerTransformModule::action),
    Loadables.SOUND_EVENT.requiredField("sound", ItemAbilityWalkerTransformModule::sound),
    LevelingValue.LOADABLE.requiredField("radius", ItemAbilityWalkerTransformModule::radius),
    ModifierCondition.TOOL_FIELD,
    ItemAbilityWalkerTransformModule::new);
  
  /** @apiNote Internal constructor, use {@link #builder(ItemAbility, SoundEvent)} */
  @Internal
  public ItemAbilityWalkerTransformModule {}
  ...
  @Override
  public RecordLoadable<ItemAbilityWalkerTransformModule> getLoader() {
    return LOADER;
  }
  ...
  public static class Builder extends ModuleBuilder.Stack<Builder> implements LevelingValue.Builder<ItemAbilityWalkerTransformModule> {
    private final ItemAbility action;
    private final SoundEvent sound;

    @Override
    public ItemAbilityWalkerTransformModule amount(float flat, float eachLevel) {
      return new ItemAbilityWalkerTransformModule(action, sound, new LevelingValue(flat, eachLevel), condition);
    }
  }
}
