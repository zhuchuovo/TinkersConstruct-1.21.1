...
public record PotionFluidEffect(float scale, TagPredicate predicate) implements FluidEffect<FluidEffectContext.Entity> {
  ...
  public float apply(FluidStack fluid, EffectLevel level, FluidEffectContext.Entity context, FluidAction action) {
    ...
    if (target != null && predicate.test(FluidStackUtil.getTag(fluid))) {
      List<MobEffectInstance> effects = new java.util.ArrayList<>();
      PotionUtils.getPotionContents(fluid).forEachEffect(effects::add);
      if (!effects.isEmpty()) {
        ...
        for (MobEffectInstance instance : effects) {
          net.minecraft.core.Holder<MobEffect> effect = instance.getEffect();
