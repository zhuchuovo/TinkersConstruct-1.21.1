...
public abstract class TinkerModule {
  ...
  protected static final SynchronizedDeferredRegister<AttachmentType<?>> ATTACHMENT_TYPES = SynchronizedDeferredRegister.create(NeoForgeRegistries.Keys.ATTACHMENT_TYPES, TConstruct.MOD_ID);
  protected static final SynchronizedDeferredRegister<MapCodec<? extends ICondition>> CONDITION_SERIALIZERS = SynchronizedDeferredRegister.create(NeoForgeRegistries.Keys.CONDITION_CODECS, TConstruct.MOD_ID);
  ...
  protected static final SynchronizedDeferredRegister<CriterionTrigger<?>> CRITERIA_TRIGGERS = SynchronizedDeferredRegister.create(Registries.TRIGGER_TYPE, TConstruct.MOD_ID);
  ...
  protected static BlockBehaviour.Properties glassBuilder(MapColor color) {
    return builder(color, SoundType.GLASS)
      ...
      .strength(0.3F).noOcclusion().isValidSpawn((state, level, pos, type) -> false)
