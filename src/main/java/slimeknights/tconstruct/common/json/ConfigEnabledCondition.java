...
public class ConfigEnabledCondition implements ICondition, LootItemCondition {
  ...
  public static final MapCodec<ConfigEnabledCondition> CODEC = Codec.STRING.fieldOf("prop").xmap(
    prop -> {
      ConfigEnabledCondition condition = PROPS.get(prop.toLowerCase(Locale.ROOT));
      if (condition == null) throw new JsonSyntaxException("Invalid property name '" + prop + "'");
      return condition;
